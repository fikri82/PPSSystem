        ...
        Try
            Select Case pAction
                ...
                Case "downloadSummary"
                    Dim psERR As String = ""
                    Dim dtProd As DataTable = GetTableETDExport(Format(DtPeriod.Value, "yyyy-MM-01"), Trim(cboAffiliate.Text), Trim(cboSupplier.Text))
...
#End Region
    ...
    Public Shared Function GetTableETDExport(ByVal pPeriod As Date, ByVal pAffiliateID As String, Optional ByRef pErr As String = "") As DataTable
        Dim ls_sql As String = ""
        ...
        Try
            ...
            Using cn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                ls_sql = " SELECT Period, AffiliateID = RTRIM(AffiliateID), SupplierID = RTRIM(SupplierID), Week, ETDVendor, ETAForwarder, ETDPort, ETAPort, ETAFactory, CutOfDate " & vbCrLf & _
                                  ...
                                  " where YEAR(Period) = " & Year(pPeriod) & " and MONTH(Period) = " & Month(pPeriod) & " "
                If pAffiliateID <> "" Then
                    ls_sql = ls_sql & "and AffiliateID = '" & pAffiliateID & "'"
                End If
