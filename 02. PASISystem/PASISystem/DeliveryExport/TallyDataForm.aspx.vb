        ...
        Try
            ...
            Using cn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                ls_sql = " SELECT TD.ShippingInstructionNo AS colInvoice, TD.ForwarderID as colForwarder, " & vbCrLf & _
                                  ...
                                  " TM.BL_AWB_No colBLAWBNo, TM.BL_AWB_Date colBLAWBDate, TM.PEB_No colPEBNo, TM.PEB_Date colPEBDate, TM.Type colType, ISNULL(SD.Price,0) Price " & vbCrLf & _
                                  " FROM dbo.Tally_Detail TD " & vbCrLf & _
                                  " INNER JOIN dbo.Tally_Master TM ON TM.AffiliateID = TD.AffiliateID  " & vbCrLf & _
                                  " AND TM.ForwarderID = TD.ForwarderID AND TM.ContainerNo = TD.ContainerNo " & vbCrLf & _
                                  " AND TM.ShippingInstructionNo = TD.ShippingInstructionNo " & vbCrLf & _
                ...
                ls_group = " GROUP BY TD.ShippingInstructionNo, TD.ForwarderID,TD.AffiliateID,TM.ContainerNo,TM.SealNo,TM.Tare,TM.Gross,TM.Vessel,TM.ContainerSize, " & vbCrLf & _
                           " TM.ETD,TM.ShippingLine,TM.DestinationPort,TM.StuffingDate,TD.PalletNo,TD.OrderNo,TD.PartNo,TD.Length,TD.Width,TD.Height,TD.M3, TD.CaseNo, TD.CaseNo2,TD.WeightPallet, TM.NamaKapal, " & vbCrLf & _
                           " TM.BL_AWB_No, TM.BL_AWB_Date, TM.PEB_No, TM.PEB_Date, TM.Type, TD.TotalBox, SD.QtyBox, SD.Price " & vbCrLf & _
                           " ORDER BY TD.ShippingInstructionNo "
    ...
    Private Sub epplusExportExcel(ByVal pFilename As String, ByVal pSheetName As String,
        ...
        Try
            ...
            With ws
                ...
                .Cells(8, 28, pData.Rows.Count + 7, 28).Style.Numberformat.Format = "@"
                .Cells(8, 29, pData.Rows.Count + 7, 29).Style.Numberformat.Format = "dd-mmm-yy"
                .Cells(8, 30, pData.Rows.Count + 7, 30).Style.Numberformat.Format = "@"
                .Cells(8, 31, pData.Rows.Count + 7, 31).Style.Numberformat.Format = "dd-mmm-yy"

                .Cells(8, 28, pData.Rows.Count + 7, 28).Style.HorizontalAlignment = Style.ExcelHorizontalAlignment.Center
                .Cells(8, 28, pData.Rows.Count + 7, 28).Style.VerticalAlignment = Style.ExcelVerticalAlignment.Center

                .Cells(8, 29, pData.Rows.Count + 7, 29).Style.HorizontalAlignment = Style.ExcelHorizontalAlignment.Center
                .Cells(8, 29, pData.Rows.Count + 7, 29).Style.VerticalAlignment = Style.ExcelVerticalAlignment.Center

                .Cells(8, 30, pData.Rows.Count + 7, 30).Style.HorizontalAlignment = Style.ExcelHorizontalAlignment.Center
                .Cells(8, 30, pData.Rows.Count + 7, 30).Style.VerticalAlignment = Style.ExcelVerticalAlignment.Center

                .Cells(8, 31, pData.Rows.Count + 7, 31).Style.HorizontalAlignment = Style.ExcelHorizontalAlignment.Center
                .Cells(8, 31, pData.Rows.Count + 7, 31).Style.VerticalAlignment = Style.ExcelVerticalAlignment.Center


                Dim rgAll As ExcelRange = .Cells(8, 1, pData.Rows.Count + 7, 32)
                        ...
                        Else
                            ...
                            For Each selectItemId As Object In selectItems
                                ...
                                If cboCreate.Text = "CREATE" Then
                                    ...
                                    "           '" & uf_GetQtyBox(Trim(pInvoiceNo), pOrder.Trim, pPart.Trim, pAff.Trim) & "')  -- POQtyBox - numeric " & vbCrLf
                                Else
                                    ls_SQL = " UPDATE dbo.Tally_Detail SET " & vbCrLf & _
                                        ...
                                        " and ContainerNo = '" & Trim(pContainerNoOld) & "' " & vbCrLf
                                End If

                                'Untuk Update BL No dn BL Date ke ShippingInstruction_Master
                                ls_SQL = ls_SQL & " UPDATE dbo.ShippingInstruction_Master SET " & vbCrLf & _
                                        " BLNo = '" & Trim(pBLAWBNo) & "', " & vbCrLf & _
                                        " BLDate = '" & Trim(pBLAWBDate) & "' " & vbCrLf & _
                                        " where ShippingInstructionNo = '" & Trim(pInvoiceNo) & "' " & vbCrLf & _
                                        " and ForwarderID = '" & Trim(pForwarder) & "' " & vbCrLf & _
                                        " and AffiliateID = '" & Trim(pAffiliate) & "' "
