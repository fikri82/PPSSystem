    ...
    Private Sub up_GridLoad()
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = "  SELECT *  " & vbCrLf & _
                     ...
                     "   	,ME = POM.EmergencyCls " & vbCrLf

            ls_SQL = ls_SQL + "   	,POIssueDateAffiliate = Format(POM.EntryDate,'yyyy-MM-dd') " & vbCrLf & _
                              "   	,PartNo = TRIM(POD.PartNo)   " & vbCrLf & _
                              "   	,PartName = TRIM(MP.PartName)   " & vbCrLf & _
                              "   	,QtyPO = ISNULL(POD.TotalPOQty,0)   " & vbCrLf & _
                              " 	,MOQ = ISNULL(POD.POMOQ,MPM.MOQ) " & vbCrLf & _
                              "  	,QtyBox = ISNULL(POD.POQtyBox,MPM.QtyBox)  " & vbCrLf & _
                              "  	,MPM.BoxPallet  " & vbCrLf & _
                              "  	,VolumePallet = CASE WHEN ISNULL(POD.POQtyBox,ISNULL(MPM.QtyBox,0)) = 0 OR MPM.BoxPallet = 0  THEN 0 ELSE (POD.TotalPOQty/ISNULL(POD.POQtyBox,MPM.QtyBox))/MPM.BoxPallet END  " & vbCrLf & _
                              "   	,ETDSupp = POM.ETDVendor1 " & vbCrLf & _
                              "   	,ScheduleETAWH = POM.ETDVendor1 " & vbCrLf & _
                              "   	,ActualETDSupplier = DSM.DeliveryDate   " & vbCrLf

            ls_SQL = ls_SQL + " 	,ActualETAWH = RFM.ReceiveDate " & vbCrLf & _
                              "   	,SupplierDNNo = TRIM(DSM.SuratJalanNo) " & vbCrLf & _
                              " 	,SupplierDeliveryQty = DSD.DOQty " & vbCrLf & _
                              " 	,BoxNoFrom = RB.Label1 " & vbCrLf & _
                              " 	,BoxNoTo = RB.Label2 " & vbCrLf & _
                              " 	,ReceiveDate = RFM.ReceiveDate " & vbCrLf & _
                              " 	,PasiReceivingQty = RB.Box * ISNULL(POD.POQtyBox,MPM.QtyBox) " & vbCrLf & _
                              "  	,Remaining = (ISNULL(POD.TotalPOQty,0) -  " & vbCrLf & _
                              "      ( " & vbCrLf & _
                              "          Select ISNULL(SUM(Box * ISNULL(PDE.POQtyBox,MPM.QtyBox)), 0)  " & vbCrLf & _
                              "          FROM ReceiveForwarder_DetailBox RD  " & vbCrLf

            ls_SQL = ls_SQL + "          LEFT JOIN ReceiveForwarder_Master RM ON RM.AffiliateID = RD.AffiliateID   " & vbCrLf & _
                              ...
                              "          LEFT JOIN po_detail_Export PDE ON PDE.PONO = RM.PONO AND PDE.AffiliateID = RM.AffiliateID AND PDE.SupplierID = RM.SupplierID AND PDE.PartNo = RD.PartNo AND RM.OrderNo = PDE.OrderNo1 " & vbCrLf & _
                              ...
                              "            AND RD.OrderNo = POD.OrderNo1 " & vbCrLf & _
                              "            AND RD.SupplierID = POD.SupplierID " & vbCrLf & _
                              "            AND RD.AffiliateID = POD.AffiliateID " & vbCrLf & _
                              "            AND RD.PartNo = POD.PartNo " & vbCrLf & _
                              "            AND RM.ReceiveDate <= RFM.ReceiveDate " & vbCrLf & _
                              "      )) * (-1) " & vbCrLf & _
                              "  	,Remaining1 = (ISNULL(POD.TotalPOQty,0) -  " & vbCrLf & _
                              "      ( " & vbCrLf & _
                              "          Select ISNULL(SUM(Box * ISNULL(PDE.POQtyBox,MPM.QtyBox)), 0)  " & vbCrLf & _
                              "          FROM ReceiveForwarder_DetailBox RD  " & vbCrLf
            ...
            ls_SQL = ls_SQL + "  FROM PO_Master_Export POM " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + " 										AND RFD.SupplierID = RFM.SupplierID " & vbCrLf & _
                              ...
                              "  LEFT JOIN (Select SuratJalanNo,SupplierID,AffiliateID,PONo,PartNo,OrderNo,SUM(DOQty) DOQty" & vbCrLf & _
                              "             From DOSupplier_Detail_Export " & vbCrLf & _
                              "             Group By SuratJalanNo,SupplierID,AffiliateID,PONo,PartNo,OrderNo " & vbCrLf & _
                              "  ) DSD ON DSM.SuratJalanNo = DSD.SuratjalanNo   " & vbCrLf & _
                              "                                         AND DSM.AffiliateID = DSD.AffiliateID   " & vbCrLf

            ls_SQL = ls_SQL + "                                         AND DSM.SupplierID = DSD.SupplierID   " & vbCrLf & _
                              "                                         AND DSM.PONO = DSD.PONO   " & vbCrLf & _
                              "                                         AND RFD.PartNo = DSD.PartNo   " & vbCrLf & _
                              "                                         AND RFD.PONO = DSD.PONO  " & vbCrLf & _
                              "  LEFT JOIN ReceiveForwarder_DetailBox RB ON RB.SuratJalanNo = RFD.SuratJalanNo   " & vbCrLf & _
                              "                                          AND RB.AffiliateID = RFD.AffiliateID   " & vbCrLf & _
                              "                                          AND RB.SupplierID = RFD.SupplierID   " & vbCrLf & _
                              "                                          AND RB.PartNo = RFD.PartNo   " & vbCrLf & _
                              "                                          AND RB.StatusDefect IN ('0','1')   " & vbCrLf & _
                              "                                          AND RB.PONo = RFD.PONo   " & vbCrLf & _
                              "                                          AND RB.OrderNo = RFD.OrderNo   " & vbCrLf
            ...
            ls_SQL = ls_SQL + "  LEFT JOIN MS_Parts MP ON MP.PartNo = POD.PartNo   " & vbCrLf & _
                              ...
                              "  WHERE POD.TotalPOQty > 0 " & vbCrLf & _
                              "  AND YEAR(POM.Period) > '2018' And YEAR(POM.EntryDate) > '2018'" & vbCrLf
    ...
    Private Function GetSummaryOutStanding() As DataTable
        ...
        Try
            ...
            Using cn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                ls_sql = "  SELECT Period,PONo,AffiliateID,SupplierID,ME,POIssueDateAffiliate,PartNo,PartName,QtyPO,MOQ,QtyBox,BoxPallet,VolumePallet " & vbCrLf & _
                     "  ,ETDSupp,ScheduleETAWH,ActualETDSupplier,ActualETAWH,SupplierDNNo,SupplierDeliveryQty,BoxNoFrom,BoxNoTo,ReceiveDate,PasiReceivingQty,Remaining" & vbCrLf & _
                     "  ,StatusPO = CASE WHEN Remaining1 > 0 THEN 'OVER'  " & vbCrLf & _
                     "  WHEN Remaining1 < 0 THEN 'OPEN'  " & vbCrLf & _
                     "  ELSE 'CLOSE' END  " & vbCrLf & _
                     "  FROM ( " & vbCrLf & _
                ...
                ls_sql = ls_sql + "   	,POIssueDateAffiliate = Format(POM.EntryDate,'yyyy-MM-dd') " & vbCrLf & _
                                  ...
                                  " 	,MOQ = ISNULL(POD.POMOQ,MPM.MOQ) " & vbCrLf & _
                                  "  	,QtyBox = ISNULL(POD.POQtyBox,MPM.QtyBox)  " & vbCrLf & _
                                  "  	,BoxPallet = ISNULL(MPM.BoxPallet,0)  " & vbCrLf & _
                                  "  	,VolumePallet = ISNULL(CASE WHEN ISNULL(POD.POQtyBox,ISNULL(MPM.QtyBox,0)) = 0 OR MPM.BoxPallet = 0  THEN 0 ELSE (POD.TotalPOQty/ISNULL(POD.POQtyBox,MPM.QtyBox))/MPM.BoxPallet END,0)  " & vbCrLf & _
                                  "   	,ETDSupp = POM.ETDVendor1 " & vbCrLf & _
                                  "   	,ScheduleETAWH = POM.ETDVendor1 " & vbCrLf & _
                                  "   	,ActualETDSupplier = DSM.DeliveryDate   " & vbCrLf
                ...
                ls_sql = ls_sql + " 	,ActualETAWH = RFM.ReceiveDate " & vbCrLf & _
                                  "   	,SupplierDNNo = TRIM(DSM.SuratJalanNo) " & vbCrLf & _
                                  " 	,SupplierDeliveryQty = ISNULL(DSD.DOQty,0) " & vbCrLf & _
                                  " 	,BoxNoFrom = RB.Label1 " & vbCrLf & _
                                  " 	,BoxNoTo = RB.Label2 " & vbCrLf & _
                                  " 	,ReceiveDate = RFM.ReceiveDate " & vbCrLf & _
                                  " 	,PasiReceivingQty = ISNULL(RB.Box * ISNULL(POD.POQtyBox,MPM.QtyBox),0) " & vbCrLf & _
                                  "  	,Remaining = (ISNULL(POD.TotalPOQty,0) -  " & vbCrLf & _
                                  "      ( " & vbCrLf & _
                                  "          Select ISNULL(SUM(Box * ISNULL(PDE.POQtyBox,MPM.QtyBox)), 0)  " & vbCrLf & _
                                  "          FROM ReceiveForwarder_DetailBox RD  " & vbCrLf
    ...
    Private Sub epplusExportExcel(ByVal pFilename As String, ByVal pSheetName As String,
        ...
        Try
            ...
            With ws
                ...
                Dim rgAll As ExcelRange = .Cells(8, 1, pData.Rows.Count + 7, 25)
