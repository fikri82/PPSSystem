...
#Region "Procedures"
    ...
    Private Function uf_SQL() As String
        ...
        ls_SQL = "   SELECT *   " & vbCrLf & _
                  ...
                  "    	,SupplierID=TRIM(POM.SupplierID)"
        ...
        ls_SQL = ls_SQL + "    	,PASISendAffiliateDate  = Format(POM.EntryDate,'yyyy-MM-dd') " & vbCrLf & _
                          ...
                          "   	,VolumePallet = case When ISNULL(MPM.BoxPallet,'0') = '0' Then '0' Else (POD.TotalPOQty/ISNULL(POD.POQtyBox,MPM.QtyBox))/MPM.BoxPallet End" & vbCrLf & _
                          "    	,ETDSupp = POM.ETDVendor1    " & vbCrLf & _
                          "    	,ETAAff = POM.ETAFactory1 " & vbCrLf & _
                          "    	,PASIDeliveryDate = DSM.DeliveryDate " & vbCrLf & _
                          "    	,PASISuratJalanNo = ISNULL(TRIM(COALESCE(DSM.SuratJalanNo,RFM.SuratJalanNo)),'') "

        ls_SQL = ls_SQL + "  	,PASIInvoiceNo = ISNULL(TRIM(SD.ShippingInstructionNo),'') " & vbCrLf & _
                          "  	,PASIDeliveryQty = ISNULL(SD.ShippingQty,0)   " & vbCrLf & _
                          "   	,Remaining = (ISNULL(POD.TotalPOQty,0) -   " & vbCrLf & _
                          "       (  " & vbCrLf & _
                          "           Select ISNULL(SUM(ShippingQty), 0)   " & vbCrLf & _
                          "           FROM ShippingInstruction_Detail RRD   " & vbCrLf & _
    ...
    Private Sub epplusExportExcel(ByVal pFilename As String, ByVal pSheetName As String,
        ...
        Try
            ...
            With ws
                ...
                '.Cells(8, 16, pData.Rows.Count + 7, 16).Style.Numberformat.Format = "#,##0"
                ...
                Dim rgAll As ExcelRange = .Cells(8, 1, pData.Rows.Count + 7, 19)
